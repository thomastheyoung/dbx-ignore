...
impl StatusInfo {
    pub fn gather() -> Result<Self> {
        ...
        let ignored_files: Vec<PathBuf> = all_files.iter()
            .filter(|f| *file_status.get(*f).unwrap_or(&false))
            .cloned()
            .collect();
            
        let non_ignored_files: Vec<PathBuf> = all_files.iter()
            .filter(|f| !*file_status.get(*f).unwrap_or(&false))
            .cloned()
            .collect();
    ...
    pub fn display(&self, verbose: bool) -> Result<()> {
        println!("{}", "═══════════════════════════════════════════════════════════".blue());
        println!("{} {}", "Status Report for:".blue().bold(), self.current_path.display());
        println!("{}", "═══════════════════════════════════════════════════════════".blue());
        println!();
        ...
        // File counts
        println!("{} {} files total", "Files:".yellow().bold(), self.total_files);
        println!("   {} {} files have ignore markers", 
            "✓".green(), 
            self.ignored_files.len().to_string().green()
        );
        println!("   {} {} files don't have ignore markers", 
            "✗".red(), 
