        ...
        tokio::select! {
            ...
            _ = debounce_timer.tick() => {
                ...
                if !events.is_empty() {
                    ...
                    if let Err(e) = perform_scan(&repo_root) {
...
fn should_trigger_rescan(event: &Event) -> bool {
    match event.kind {
        EventKind::Create(_) | EventKind::Modify(_) | EventKind::Remove(_) => {
            // Check if any path is a .gitignore file or within the .git directory
...
fn perform_scan(repo_root: &Path) -> Result<()> {
    // Load tracked files
