...
mod macos_tests {
    ...
    fn test_remove_attribute_on_file_without_attribute() {
        ...
        // Removing non-existent attribute should succeed (no-op)
        let result = MacOSHandler::remove_attribute(&test_file, "com.dropbox.ignored");
        // Note: This might fail on some systems, which is expected behavior
        // The actual behavior depends on the xattr implementation
        match result {
            Ok(_) => {
