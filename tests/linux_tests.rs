...
mod linux_tests {
    ...
    fn test_has_attribute_on_regular_file() {
        ...
        let result = LinuxHandler::has_attribute(&test_file, "user.com.apple.fileprovider.ignore#P");
        assert!(result.is_ok());
        assert!(!result.unwrap());
    }
    ...
    fn test_remove_attribute_on_file_without_attribute() {
        ...
        // Removing non-existent attribute should succeed (no-op) or fail gracefully
        let result = LinuxHandler::remove_attribute(&test_file, "user.com.dropbox.ignored");
        match result {
            Ok(_) => {
                // Success is fine - attribute didn't exist anyway
            }
            Err(_) => {
                // Failure is also acceptable for non-existent attributes
                // This depends on the underlying xattr implementation
            }
        }
    }
    ...
    fn test_linux_specific_user_prefix() {
        ...
        for attr in attrs {
            assert!(attr.starts_with("user."), 
                "Linux extended attribute '{}' should start with 'user.' prefix", attr);
        }
    }
