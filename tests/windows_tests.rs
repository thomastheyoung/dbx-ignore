...
mod windows_tests {
    ...
    fn test_remove_attribute_on_file_without_stream() {
        ...
        // Removing non-existent stream should fail gracefully
        let result = WindowsHandler::remove_attribute(&test_file, "com.dropbox.ignored");
        match result {
            Ok(_) => {
                // Success is unexpected but acceptable if implementation changes
            }
            Err(_) => {
                // Failure is expected for non-existent streams
            }
        }
    }
    ...
    fn test_windows_specific_stream_behavior() {
        ...
        for attr in attrs {
            assert!(!attr.starts_with("user."), 
                "Windows stream '{}' should not have 'user.' prefix", attr);
            assert!(!attr.is_empty(), 
                "Windows stream name should not be empty");
        }
    }
