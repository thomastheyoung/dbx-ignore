...
fn test_gitignore_implementation_matches_git() -> Result<()> {
    ...
    let git_set: HashSet<PathBuf> = git_ignored
        .into_iter()
        .filter_map(|p| p.strip_prefix(temp_path).ok().map(|p| p.to_path_buf()))
        .collect();

    let our_set: HashSet<PathBuf> = our_ignored
        .into_iter()
        .filter_map(|p| p.strip_prefix(temp_path).ok().map(|p| p.to_path_buf()))
        .collect();
    ...
    let only_in_ours: Vec<_> = our_expanded
        .difference(&git_set)
...
fn test_gitignore_edge_cases() -> Result<()> {
    ...
    let git_normalized: Vec<_> = git_ignored
        .iter()
        .map(|p| p.canonicalize().unwrap_or_else(|_| p.clone()))
        .collect();
    let our_normalized: Vec<_> = our_ignored
        .iter()
        .map(|p| p.canonicalize().unwrap_or_else(|_| p.clone()))
        .collect();
    ...
    let git_has_build_dir = git_ignored
        .iter()
        .any(|p| p.ends_with("build") || p.ends_with("build/"));
    let our_has_build_dir = our_ignored
        .iter()
        .any(|p| p.ends_with("build") || p.ends_with("build/"));
